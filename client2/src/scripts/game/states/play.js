...
module.exports = function(Game) {
    ...
    function initGame() {
        ...
        this.pickupTimer = this.game.time.create(false);
    ...
    function spawnPickups() {
        this.generatePickups();
        var time = this.game.rnd.realInRange(this.coolMin, this.coolMax);
        console.log("spawn pickups again in: " + time + " seconds");
        this.pickupTimer.add(Phaser.Timer.SECOND * time, this.spawnPickups, this);
    }

    function random(max) {
        return this.game.rnd.realInRange(0, max);
    }

    function generatePickups() {
        for (let i = 0; i < 3; i++) {
            if (Phaser.Math.chanceRoll()) {
                var pickup = new Game.Prefabs.PickupSpeedUp(this.game, this.random(this.game.width), this.random(this.game.height));
                this.game.add.existing(pickup);
                //debug(pickup);
                this.pickups.push(pickup);
            }

            // var pickup2 = new Game.Prefabs.PickupSpeedUp(this.game,this.random(this.game.width), this.random(this.game.height));
            // this.game.add.existing(pickup2);
            // //debug(pickup2);
            // this.pickups.push(pickup2);

            if (Phaser.Math.chanceRoll()) {
                var pickup3 = new Game.Prefabs.PickupSpeedDown(this.game, this.random(this.game.width), this.random(this.game.height));
    ...
    function shootBullet() {
        ...
        var bulletInfo = this.hero.getBulletData();

        bulletInfo.forEach(bulletInfo => {
            var bullet, bulletPosY;
            bullet = this.bullets.getFirstExists(false);
            if (bullet) {

                var position = new Phaser.Point(this.hero.x, this.hero.y);
                var rotation = this.hero.rotation + bulletInfo.rotation;
                var offset = new Phaser.Point(bulletInfo.position.x, bulletInfo.position.y);
                offset.rotate(0, 0, rotation + Math.PI / 2);
                Phaser.Point.add(position, offset, position);
